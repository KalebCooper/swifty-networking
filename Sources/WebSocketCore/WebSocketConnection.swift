...
///
/// Receive yields complete messages; nil means backend-reported closure and closeInfo must already
...
public protocol WebSocketConnection: Sendable {
  /// Backend-reported closure metadata, available before receive ends normally.
  ...
  func cancel()
  /// Requests orderly shutdown and returns backend-reported closure metadata.
  ///
  /// The shared caller supplies validated arguments and bounds the operation with a deadline.
  /// Completion need not establish peer acknowledgement; metadata may reflect the local request.
  ...
  /// Receives one complete application message, or nil after backend-reported closure.
